...
export class CustomerDebitService {
    ...
    async countDebit(
        ...
        Object.keys(filter).forEach((item, index) => {
            queryString += `AND CustomerDebit.${item} like '%${filter[item]}%' ${Object.keys(filter).length - 1 === index ? '' : 'AND '}`;
        });
        ...
        if (!allowToSeeAll) {
            if (isEmployee) {queryString += ` AND CustomerDebit.sale = ${currentUser.id}`;}
