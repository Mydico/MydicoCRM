...
export class FileController {
  ...
  @UseInterceptors(
    FilesInterceptor('file', 20, {
      ...
      fileFilter: (req: any, file: any, cb: any) => {
        let filetypes = /jpeg|jpg|png|xml|csv|webp|bitmap|doc|docx|xls|pdf|xlsx|mp3|/
  ...
  @UseInterceptors(
    FileInterceptor('upload', {
      ...
      fileFilter: (req: any, file: any, cb: any) => {
        let filetypes = /jpeg|jpg|png|xml|csv|webp|bitmap|doc|docx|xls|pdf|xlsx/;
        let mimetype = filetypes.test(file.mimetype);
        let extname = filetypes.test(path.extname(file.originalname).toLowerCase());
        const mineType = file.originalname.split('.')[1];
        if (mimetype && extname) {
  ...
  async downLoadTemplate(@Res() res: Response, @Req() req : Request): Promise<any> {
    ...
    let filePath = `${directory}/${req.query.name}`;
    let fileName = path.basename(filePath);
    res.setHeader('Content-Type', 'application/octet-stream')
    res.setHeader('Content-disposition', 'inline; filename=' + fileName);
    let stream = createReadStream(filePath)
