...
export const getCustomerBySale = createAsyncThunk('api/customers/find-by-sale', async (params = {page: 0, size: 50, sort: 'createdDate,DESC'}, thunkAPI) => {
  try {
    const result = await axios.get('api/customers/find-by-sale', {params: params});
    return {data: result.data, total: result.headers['x-total-count']};
  } catch (error) {
    return thunkAPI.rejectWithValue(error.response.data);
  }
