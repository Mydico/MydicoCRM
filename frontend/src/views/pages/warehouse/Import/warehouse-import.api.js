...
export const getWarehouseImport = createAsyncThunk('api/store-inputs', async (params = { page: 0, size: 20, sort: 'createdDate,desc' }, thunkAPI) => {
  try {
...
export const getWarehouseExport = createAsyncThunk('api/store-inputs/export', async (params = { page: 0, size: 20, sort: 'createdDate,desc' }, thunkAPI) => {
  try {
...
export const getDetailWarehouseImport = createAsyncThunk('api/detail/store-inputs', async (userId, thunkAPI) => {
  try {
    const result = await axios.get('api/store-inputs/'+ userId);
    return result.data
  } catch (error) {
    return thunkAPI.rejectWithValue(error.response.data);
  }
