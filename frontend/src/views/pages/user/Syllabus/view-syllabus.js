...
import { creatingSyllabus, getSyllabus } from './syllabus.api';
...
const { selectAll: selectAllSyllabuss } = globalizedSyllabusSelectors;
...
const CreateSyllabus = () => {
  ...
  const syllabus = useSelector(selectAllSyllabuss);
  const [selectedBranches, setSelectedBranches] = useState([]);
  const [selectedGroupPermission, setSelectedGroupPermission] = useState([]);
  const [reportSyllabus, setReportSyllabus] = useState([]);
  ...
  useEffect(() => {
    ...
    dispatch(getSyllabus({ page: 0, size: 200, sort: 'createdDate,DESC', dependency: true }));
    return () => {
      dispatch(reset());
    };
  }, []);
  ...
  const onSubmit = (values, { resetForm }) => {
    ...
    values.reportSyllabus = reportSyllabus ? JSON.stringify(reportSyllabus.map(item => item.value)) : '[]';
          ...
          {({
            ...
            <CForm onSubmit={handleSubmit} noValidate name="simpleForm">
              ...
              <CFormGroup>
                ...
                <Select
                  name="syllabus"
                  onChange={setReportSyllabus}
                  isMulti
                  value={reportSyllabus}
                  placeholder="Chọn chi nhánh"
                  options={syllabus.map(item => ({
                    value: item.id,
                    label: item.name
